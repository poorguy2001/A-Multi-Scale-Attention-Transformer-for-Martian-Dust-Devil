...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, WIoU=False,
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or mpdiou or WIoU or smiou:
        ...
        if CIoU or DIoU or EIoU or SIoU or mpdiou or WIoU or ShapeIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                d22 = ((b2_x2 - b1_x2) - (b2_y2 - b1_y2)) ** 2
                # 添加形状惩罚
                omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2 + eps)  # 宽度相对差异
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2 + eps)  # 高度相对差异
                shape_cost = torch.pow(1 - torch.exp(-omiga_w), 4) + torch.pow(1 - torch.exp(-omiga_h), 4)
                return iou - ((d12 + d22) / (cw2 + ch2)) - beta * shape_cost #改
...
class Metric(SimpleClass):
    ...
    def mean_results(self):
        ...
        return [self.mp, self.mr, self.map50, self.map,self.mean_f1]

    def class_result(self, i):
        """Class-aware result, return p[i], r[i], ap50[i], ap[i]."""
        f1_score = 2 * (self.p[i] * self.r[i]) / (self.p[i] + self.r[i]) if self.p[i] + self.r[i] else 0.0  # F1 score
        return self.p[i], self.r[i], self.ap50[i], self.ap[i],f1_score
